...
app.post("/code-practice", (req, res) => {
  try {
    ...
    let file_path_user =
      ...
      req.body.user_id +
      "/code_practice/" +
      req.body.practice_code_id +
      "/user";
    let file_path_solution =
      defaultPath +
      "/" +
      req.body.user_id +
      "/code_practice/" +
    ...
    if (req.body.run === true) {
      ...
      try {
        ...
        res
          .status(400)
          .send({ error: { message: error, title: "TestCase không hợp lệ" } });
      }
      let return_value = [];
      let return_solution = [];
      let error = false;
      ...
      input_output.every(async (element) => {
        ...
        if (
          ((count === total && count_solution === total) || error === true) &&
          flag === false
        ) {
          if (error) {
            res.status(400).send({
              error: {
                message: utils.customErrorReturn(return_value),
                title: "Lỗi Runtime",
              },
            });
            flag = true;
