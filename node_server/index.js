    ...
    try {
        ...
        PythonShell.run(file_path + '/' + file_name, options)
            ...
            .catch((err) => {
                res.status(400).send({
                    error: {
                        message: utils.customErrorReturn(errors.join('\r\n')),
                        title: 'Lỗi Runtime',
                    },
                });
            });
    ...
    try {
        ...
        if (req.body.run === true) {
            ...
            input_output.every(async (element) => {
                ...
                await PythonShell.run(file_path_user + '/' + 'main.py', options_user)
                    .then((messages) => {
                        count++;
                        return_value.push({
                            id: element.id,
                            value: { input: element.original_input, output: messages[0] },
                        });
                    })
                    .catch((err) => {
                ...
                let options_solution = {
                    mode: 'text',
                    pythonOptions: ['-u'],
                    args: [JSON.stringify(args)],
                };
                await PythonShell.run(file_path_solution + '/' + 'main.py', options_solution)
                    .then((messages) => {
                        count_solution++;
                        return_solution.push({
                            id: element.id,
                            value: { input: element.original_input, output: messages[0] },
                        });
                    })
